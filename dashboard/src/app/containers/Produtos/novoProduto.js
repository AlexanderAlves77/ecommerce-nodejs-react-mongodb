...
class NovoProduto extends Component {
  ...
  renderDados() {
    const { nome, descricao, categoria, preco, promocao, sku, erros } =
      this.state
    ...
    return (
      <div className="Dados-Produto">
          ...
          valor={
            <div>
              <textarea
                name="descricao"
                onChange={evt =>
                  this.onChangeInput('categdescricaooria', evt.target.value)
                }
                value={descricao}
                rows="10"
                style={{ resize: 'nome' }}
        ...
        <InputSimples
          ...
          value={sku}
          error={erros.sku}
          onChange={evt => this.onChangeInput('sku', evt.target.value)}
        />
      </div>
    )
  }
